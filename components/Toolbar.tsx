...
import { ToolSettings, BubbleType, FontName, FONTFAMILYMAP, Bubble, MIN_TAIL_BASE_WIDTH, MIN_TAIL_LENGTH, MIN_DOT_COUNT, MIN_DOT_SIZE } from '../types.ts';
...
const fontOptions = Object.keys(FontName).map(key => ({
  ...
  fontFamilyStyle: FONTFAMILYMAP[FontName[key as keyof typeof FontName] as FontName],
}));
...
export const Toolbar: React.FC<ToolbarProps> = ({
  ...
  return (
    <div className="mb-6 p-4 border border-gray-300 rounded-lg bg-white shadow-md space-y-6">
      ...
      <div>
        <h2 className="text-lg font-semibold mb-3 text-gray-700 font-['Bangers']">Project</h2>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <ToolButton onClick={onSaveProject} className="bg-blue-500 hover:bg-blue-600" disabled={!isProjectLoaded || isSaving} title={!isProjectLoaded ? "Chargez une image d'abord" : ""}>
            Sauvegarder Projet
          </ToolButton>
          <ToolButton onClick={() => onSave('png')} className="bg-green-600 hover:bg-green-700" disabled={!isProjectLoaded || isSaving} title={isSaving ? "Exportation..." : ""}>
            Exporter PNG
          </ToolButton>
          <ToolButton onClick={() => onSave('jpeg')} className="bg-teal-600 hover:bg-teal-700" disabled={!isProjectLoaded || isSaving} title={isSaving ? "Exportation..." : ""}>
            Exporter JPG
          </ToolButton>
      ...
      <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2">
            {fontOptions.map(font => (
              <ToolButton
                ...
                onClick={() => onSettingChange({ activeFontFamily: font.value })}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          <div>
            <h3 className="font-medium mb-1 text-sm text-gray-600">Taille du texte {selectedBubble ? `${selectedBubble.fontSize}px` : `${settings.activeFontSize}px`}</h3>
            <input
              type="range"
              min="5"
              max="40"
          ...
          <div className="flex items-center gap-4">
            <div>
              <label htmlFor="textColor" className="font-medium mb-1 text-sm text-gray-600 block">
                Texte
              </label>
              <input
                type="color"
            ...
            {BUBBLE_TYPES_WITH_BORDERS.includes(settings.activeBubbleType) && (
              <div>
                <label htmlFor="borderColor" className="font-medium mb-1 text-sm text-gray-600 block">
                  Bordure
                </label>
                <input
                  type="color"
        ...
        {(settings.activeBubbleType === BubbleType.SpeechDown ||
          settings.activeBubbleType === BubbleType.SpeechUp ||
          settings.activeBubbleType === BubbleType.Whisper) && (
            ...
            <div className="p-2 border rounded border-gray-200 bg-gray-50">
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                <input
                  type="number"
                  id="tailLength"
                  value={settings.defaultTailLength}
                  onChange={e => onSettingChange({ defaultTailLength: Math.max(MIN_TAIL_LENGTH, parseInt(e.target.value)) })}
                  className="w-16 p-1 border rounded text-xs"
                  min={MIN_TAIL_LENGTH.toString()}
                />
                ...
                <input
                  type="number"
                  id="tailBaseWidth"
                  value={settings.defaultTailBaseWidth}
                  onChange={e => onSettingChange({ defaultTailBaseWidth: Math.max(MIN_TAIL_BASE_WIDTH, parseInt(e.target.value)) })}
                  className="w-16 p-1 border rounded text-xs"
                  min={MIN_TAIL_BASE_WIDTH.toString()}
                />
              </div>
            </div>
          </div>
          ...
          <div className="p-2 border rounded border-gray-200 bg-gray-50">
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <input
                type="number"
                id="dotCount"
                value={settings.defaultDotCount}
                onChange={e => onSettingChange({ defaultDotCount: Math.max(MIN_DOT_COUNT, parseInt(e.target.value)) })}
                className="w-16 p-1 border rounded text-xs"
                min={MIN_DOT_COUNT.toString()}
              />
              ...
              <input
                type="number"
                id="dotSize"
                value={settings.defaultDotSize}
                onChange={e => onSettingChange({ defaultDotSize: Math.max(MIN_DOT_SIZE, parseInt(e.target.value)) })}
                className="w-16 p-1 border rounded text-xs"
                min={MIN_DOT_SIZE.toString()}
              />
            </div>
          </div>
        )}
