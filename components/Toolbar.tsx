...
import { ToolSettings, BubbleType, FontName, FONT_FAMILY_MAP, Bubble, MIN_TAIL_BASE_WIDTH, MIN_TAIL_LENGTH, MIN_DOT_COUNT, MIN_DOT_SIZE, MIN_BORDER_WIDTH, MAX_BORDER_WIDTH } from '../types.ts';
...
const bubbleTypeOptions = [
  ...
  { label: 'Dialogue Bas (Arrondis Réduits)', value: BubbleType.SpeechDownMinimal },
  { label: 'Dialogue Haut (Arrondis Réduits)', value: BubbleType.SpeechUpMinimal },
  ...
  const ToolButton: React.FC<{ onClick?: () => void; onMouseDown?: (e: React.MouseEvent) => void; active?: boolean; children: React.ReactNode; className?: string; title?: string; disabled?: boolean; style?: React.CSSProperties }> =
    ({ onClick, onMouseDown, active, children, className = '', title, disabled, style }) => (
      <button
        ...
        disabled={disabled}
        style={style}
        className={`px-3 py-2 text-sm rounded shadow hover:bg-blue-600 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-400 disabled:opacity-50 disabled:cursor-not-allowed ${active ? 'bg-blue-700 text-white ring-2 ring-blue-300' : 'bg-blue-500 text-white'} ${className}`}
      >
        {children}
      </button>
    );
    ...
    <div className="mb-6 p-4 border border-gray-300 rounded-lg bg-white shadow-md space-y-6">
      ...
      <div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <ToolButton onClick={onSaveProject} className="bg-blue-500 hover:bg-blue-600" disabled={!isProjectLoaded || isSaving} title={!isProjectLoaded ? "Chargez une image d'abord" : "Sauvegarder le projet .json"}>
            Sauvegarder Projet
          </ToolButton>

          <ToolButton onClick={() => onSave('png')} className="bg-green-600 hover:bg-green-700" disabled={!isProjectLoaded || isSaving}>
            {isSaving ? 'Exportation...' : 'Exporter PNG'}
          </ToolButton>

          <ToolButton onClick={() => onSave('jpeg')} className="bg-teal-600 hover:bg-teal-700" disabled={!isProjectLoaded || isSaving}>
            {isSaving ? 'Exportation...' : 'Exporter JPG'}
          </ToolButton>

          <ToolButton onClick={onClear} className="bg-red-500 hover:bg-red-600" disabled={isSaving}>
            Effacer Tout
          </ToolButton>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-2">
            {bubbleTypeOptions.map(opt => (
              <ToolButton key={opt.value} onClick={() => onSettingChange({ activeBubbleType: opt.value })} active={settings.activeBubbleType === opt.value} title={opt.label}>
                {opt.label}
              </ToolButton>
            ))}
          </div>
        ...
        <div className="flex items-center gap-4">
          <div>
            <label htmlFor="textColor" className="block font-medium mb-1 text-sm text-gray-600">
              Texte
            </label>
            <input
              type="color"
          ...
          {BUBBLE_TYPES_WITH_BORDERS.includes(settings.activeBubbleType) && (
            <div>
              <label htmlFor="borderColor" className="block font-medium mb-1 text-sm text-gray-600">
                Bordure
              </label>
              <input
                type="color"
        ...
        {BUBBLE_TYPES_WITH_BORDERS.includes(settings.activeBubbleType) && (
          <div className="mt-3">
            <label htmlFor="borderWidth" className="block font-medium mb-1 text-sm text-gray-600">
              Épaisseur du trait
            </label>
            <div className="flex items-center gap-2">
              <input
                type="range"
                id="borderWidth"
                min={MIN_BORDER_WIDTH.toString()}
                max={MAX_BORDER_WIDTH.toString()}
                step="0.25"
                value={selectedBubble ? selectedBubble.borderWidth : settings.activeBorderWidth}
                onChange={(e) => onSettingChange({ activeBorderWidth: parseFloat(e.target.value) })}
                className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
                aria-label="Épaisseur du trait"
              />
              <span className="text-xs text-gray-600 w-8">
                {(selectedBubble ? selectedBubble.borderWidth : settings.activeBorderWidth).toFixed(2)}
              </span>
            </div>
          </div>
        )}

        {(settings.activeBubbleType === BubbleType.SpeechDown || settings.activeBubbleType === BubbleType.SpeechUp || settings.activeBubbleType === BubbleType.SpeechDownMinimal || settings.activeBubbleType === BubbleType.SpeechUpMinimal || settings.activeBubbleType === BubbleType.Whisper || settings.activeBubbleType === BubbleType.Thought) && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {(settings.activeBubbleType === BubbleType.SpeechDown || settings.activeBubbleType === BubbleType.SpeechUp || settings.activeBubbleType === BubbleType.SpeechDownMinimal || settings.activeBubbleType === BubbleType.SpeechUpMinimal || settings.activeBubbleType === BubbleType.Whisper) && (
              <div className="p-2 border rounded border-gray-200 bg-gray-50">
                ...
                <div className="flex items-center gap-2 text-sm">
                  <label htmlFor="tailLength" className="whitespace-nowrap">
                    Longueur:
                  </label>
                  ...
                  <label htmlFor="tailBaseWidth" className="whitespace-nowrap">
                    Largeur Base (Totale):
                  </label>
            ...
            {settings.activeBubbleType === BubbleType.Thought && (
              <div className="p-2 border rounded border-gray-200 bg-gray-50">
                ...
                <div className="flex items-center gap-2 text-sm">
                  <label htmlFor="dotCount" className="whitespace-nowrap">
                    Nombre:
                  </label>
                  ...
                  <label htmlFor="dotSize" className="whitespace-nowrap">
                    Taille Moyenne:
                  </label>
